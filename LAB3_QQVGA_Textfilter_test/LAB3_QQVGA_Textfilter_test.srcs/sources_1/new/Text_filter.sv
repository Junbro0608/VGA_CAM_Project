...
module TextFilter (
    ...
    assign sw_text_any = sw_text[0] | sw_text[1];

     always_comb begin
        if (sw_text[0])      set_idx = 1'b0;
        else if (sw_text[1]) set_idx = 1'b1;
        else                 set_idx = 1'b0;
    end
    ...
    assign is_label_area = sw_text_any && (in_col0 || in_col1 || in_col2) &&
                            (y_pixel >= ROW_Y) && (y_pixel < ROW_Y + LABEL_H);
    ...
    logic [2:0] label_idx;
    assign label_idx = {2'b0, set_idx} * 3 + cell_idx;

    assign addr = is_label_area ? (label_idx * (LABEL_W * LABEL_H) + rel_y * LABEL_W + rel_x) : '0;
    assign is_text = is_label_area && (data == 1'b1);
