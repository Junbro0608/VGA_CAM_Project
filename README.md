...
`vga_decoder`가 `de/x/y`를 생성 → `ImgRomReader`가 `ImgRom`에서 `addr`로 픽셀을 읽어 `r/g/b` 출력 → `Img_externalBar`가 같은 `x/y`를 받아 바운딩 박스/구분선을 오버레이해 최종 `r/g/b` 출력.
...
- 외곽 테두리 + 가로 구분선(80) + 세로 구분선 2개(35, 70)가 106x120 영역 안에서만 정확히 표시됨을 확인 (상단 이미지 참고)
